...
[TestFixture]
public class RoundLinkedList_Tests
{
	[Test]
	public void ShouldAllowAHead()
	{
		RoundLinkedList<int> ll = new();
	...
	public void ShouldReadForward()
	{
		...
		Assert.That(ll.ReadForward(), Is.EqualTo("[HEAD]:69 -> 78 -> 84 -> 99 -> 103 -> <HEAD>"));
	}
	...
	public void GivenSeparatorShouldReadForwardWithSeparator(string separator)
	{
		...
		Assert.That(ll.ReadForward(separator), Is.EqualTo($"[HEAD]:69{separator}78{separator}84{separator}99{separator}103{separator}<HEAD>"));
	}
}